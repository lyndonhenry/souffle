...
public:
    ...
    bool isAggregatedOrNegatedInClausesOfRelation(
            const AstRelation* headRelation, const AstRelation* bodyRelation) const {
        return aggregatedAndNegatedRelationsInClausesOfRelation.at(headRelation).count(bodyRelation);
    }

private:
    std::map<const AstRelation*, const std::set<const AstRelation*>>
            aggregatedAndNegatedRelationsInClausesOfRelation;
};
/**
...
public:
    ...
    /** Get all external predecessor relations of a given SCC that occur aggregated or negated within that
     * scc. */
    const std::set<const AstRelation*> getAggregatedAndNegatedExternalPredecessorRelations(
        ...
        for (const auto& headRelation : internalRelations) {
            for (const auto& bodyRelation : externalPredecessorRelations) {
                if (aggregatedAndNegatedRelations->isAggregatedOrNegatedInClausesOfRelation(
                            headRelation, bodyRelation)) {
