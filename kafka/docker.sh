...
function _kafka_produce_log_message() {
  local KAFKA_HOST="${1}"
  local MESSAGE="${2}"
  _kafka_produce_message "${KAFKA_HOST}" "${KAFKA_LOG_TOPIC}_${ID}" "DEBUG: ${MESSAGE}"
}
...
function _broker_begin_with_no_kafka() {
  EXE="./exe"
  INPUT="./input"
  OUTPUT="./output"
...
function _broker_end() {
  ...
  _kafka_produce_log_message "${KAFKA_HOST}" "Uploading output at broker.."
  local LOG_FILE="$(basename ${S3_EXE})-$(basename ${S3_INPUT})-${THREADS}-${ID}.log"
...
function _client_run_with_kafka_many() {
  ...
  _kafka_produce_log_message "${KAFKA_HOST}" "Downloading exe at client..."
  aws s3 cp "${S3_EXE}" "${EXE}"
  chmod +x "${EXE}"
  _kafka_produce_log_message "${KAFKA_HOST}" "Waiting for topics at client..."
  _kafka_wait_for_topic "${KAFKA_HOST}" "OK_${ID}"
