...
TIMEOUTS_FILE="timeouts_azure.txt"
...
function _run_experiment() {
  ...
  then
    ...
    local SECRET_DOCKER_COMPOSE_FILE="${HOME}/.secret-azure/docker-compose.yml"
    ...
    # deploy the stack
    kubectl apply -f ${SECRET_DOCKER_COMPOSE_FILE} 
    ...
    kubectl delete -f ${SECRET_DOCKER_COMPOSE_FILE} 
...
function _main() {
  ...
  for DOCKER_COMPOSE_FILE in $(find "${ROOT}/k8s/example" -name "*.yml")
  do
