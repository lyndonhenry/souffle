...
void Synthesiser::emitCode(std::ostream& out, const RamStatement& stmt){
    class CodeEmitter : public RamVisitor<void, std::ostream&>{private : Synthesiser & synthesiser;
...
void visitDrop(const RamDrop& drop, std::ostream& out) override {
    ...
    if (Global::config().get("engine") == "mpi") {
        ...
        os << "for (const auto dependentStratum : drop.getDependentStrata()) {";
        os << "const auto dependentProcess = souffle::mpi::rankOf(dependentStratum);";
        os << "if (currentProcess == dependentProcess) {";
        os << "return false;";
        os << "}";
        os << "}";
        os << "return true;";
...
#endif
    {
        out << "{" out << "if (!isHintsProfilingEnabled() && (performIO || " << drop.getRelation().isTemp()
            << ")) ";
...
void visitSend(const RamSend& send, std::ostream& os) override {
    ...
    for (const auto stratum : send.getDestinationStrata()) {
        os << "const auto otherProcess = souffle::mpi::rankOf(" << stratum << ");" os
           << "if (currentProcess != otherProcess) {" {os << "destinationStrata.insert(otherProcess);"} os
           << "}";
    }
