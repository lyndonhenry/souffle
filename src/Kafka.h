...
public:
    ...
    static void pollProducerUntilEmpty(RdKafka::Producer* producer, int timeout_ms) {
        assert(producer);
...
public:
    ...
    void run(souffle::SouffleProgram* souffleProgram, std::size_t stratumIndex) const {
        ...
        auto kafkaClient = detail::KafkaClient(brokers);
        auto haltPayload = std::vector<souffle::RamDomain>({std::numeric_limits<souffle::RamDomain>::max()});

        const auto createProducerTopics = [&](const std::unordered_set<std::string>& topicNames) {
            std::vector<RdKafka::Topic*> topics;
            for (const auto& topicName : topicNames) {
                topics.push_back(kafkaClient.createProducerTopic(topicName));
            }
            return topics;
        };

        const auto deleteProducerTopics = [&](std::vector<RdKafka::Topic*>& topics) {
            for (RdKafka::Topic* topic : topics) {
                kafkaClient.deleteTopic(topic);
            }
        };

        const auto produceOnTopics = [&](std::vector<RdKafka::Topic*>& topics) {
            for (RdKafka::Topic* topic : topics) {
                auto relationWrapper = programWrapper.createRelationWrapper(topic->name());
#ifdef DEBUG
                // @TODO
                BREAKPOINT;
                {
                    std::stringstream ss;
                    ss << "souffle#"
                       << "produceOnTopics-" << __LINE__ << topic->name() << " ";
                    kafkaClient.debug(ss, relationWrapper.getRelationData());
                }
#endif
                auto relationData = relationWrapper.getRelationData();
                kafkaClient.produce(topic, relationData);
            }
        };

        const auto produceHaltOnTopics = [&](std::vector<RdKafka::Topic*>& topics) {
            for (RdKafka::Topic* topic : topics) {
                auto haltPayload =
                        std::vector<souffle::RamDomain>({std::numeric_limits<souffle::RamDomain>::max()});
                kafkaClient.produce(topic, haltPayload);
            }
        };

        const auto produceUntilEmpty = [&]() { kafkaClient.pollProducerUntilEmpty(1000); };

        const auto createConsumerTopics = [&](const std::unordered_set<std::string>& topicNames) {
            std::vector<RdKafka::Topic*> topics;
            for (const auto& topicName : topicNames) {
                topics.push_back(kafkaClient.createConsumerTopic(topicName));
            }
            return topics;
        };

        const auto deleteConsumerTopics = [&](std::vector<RdKafka::Topic*>& topics) {
            for (RdKafka::Topic* topic : topics) {
                kafkaClient.deleteTopic(topic);
            }
        };

        const auto startConsumerOnTopics = [&](std::vector<RdKafka::Topic*>& topics) {
            for (RdKafka::Topic* topic : topics) {
                kafkaClient.startConsumer(topic);
            }
        };

        const auto stopConsumerOnTopics = [&](std::vector<RdKafka::Topic*>& topics) {
            for (RdKafka::Topic* topic : topics) {
                kafkaClient.stopConsumer(topic);
            }
        };

        const auto consumeOnTopics = [&](std::vector<RdKafka::Topic*>& topics) {
            for (RdKafka::Topic* topic : topics) {
                std::vector<souffle::RamDomain> newRelationData;
                kafkaClient.consume(topic, newRelationData);
                auto relationWrapper = programWrapper.createRelationWrapper(topic->name());
#ifdef DEBUG
                // @TODO
                BREAKPOINT;
                {
                    std::stringstream ss;
                    ss << "souffle#"
                       << "consumeOnTopics-" << __LINE__ << " " << topic->name() << " ";
                    kafkaClient.debug(ss, relationWrapper.getRelationData());
                }
#endif
                auto relationData = relationWrapper.getRelationData();
        ...
        const auto consumeUntilHaltOnTopics = [&](std::vector<RdKafka::Topic*>& topics) {
            for (RdKafka::Topic* topic : topics) {
                ...
                BREAKPOINT;
                {
                    std::stringstream ss;
                    ss << "souffle#"
                       << "consumeUntilHaltOnTopics-" << __LINE__ << " " << topic->name() << " ";
                    kafkaClient.debug(ss, relationWrapper.getRelationData());
                }
#endif
                auto relationData = relationWrapper.getRelationData();
        ...
        const auto produceToMaster = [&]() {
            auto topic = kafkaClient.createProducerTopic("default");
            std::vector<souffle::RamDomain> payload;
            kafkaClient.produce(topic, payload);
            kafkaClient.pollProducerUntilEmpty(1000);
            kafkaClient.deleteTopic(topic);
        };

        const auto consumeFromSlaves = [&](std::size_t count) {
            auto topic = kafkaClient.createConsumerTopic("default");
            kafkaClient.startConsumer(topic);
            while (count > 0) {
                std::vector<souffle::RamDomain> payload;
                kafkaClient.consume(topic, payload);
                --count;
            }
            kafkaClient.stopConsumer(topic);
            kafkaClient.deleteTopic(topic);
        ...
        const auto beginConsumeFromMaster = [&]() {
            auto topic = kafkaClient.createConsumerTopic("default");
            kafkaClient.startConsumer(topic);
            return topic;
        };

        const auto endConsumeFromMaster = [&](RdKafka::Topic* topic) {
            while (true) {
                std::vector<souffle::RamDomain> payload;
                kafkaClient.consume(topic, payload);
        ...
        if (stratumName == std::to_string(static_cast<std::size_t>(-1))) {
            auto consumerTopics = createConsumerTopics(programMetadata_.outputRelationNames());
            startConsumerOnTopics(consumerTopics);
            consumeFromSlaves(programMetadata_.stratumNames().size());
            produceToSlaves();
            programWrapper.printAll(programMetadata_.outputPath());
            programWrapper.loadAll(programMetadata_.inputPath());
            auto producerTopics = createProducerTopics(programMetadata_.inputRelationNames());
            produceOnTopics(producerTopics);
            produceUntilEmpty();
            deleteProducerTopics(producerTopics);
            consumeUntilHaltOnTopics(consumerTopics);
            stopConsumerOnTopics(consumerTopics);
            deleteConsumerTopics(consumerTopics);
            programWrapper.printAll(programMetadata_.outputPath());
        } else {
            const auto stratumMetadata = programMetadata_.createStratumMetadata(stratumName);
            auto consumerInputTopics = createConsumerTopics(stratumMetadata.inputRelationNames());
            auto consumerConsumedTopics = createConsumerTopics(stratumMetadata.consumedRelationNames());
            startConsumerOnTopics(consumerInputTopics);
            startConsumerOnTopics(consumerConsumedTopics);
            const auto masterTopic = beginConsumeFromMaster();
            produceToMaster();
            endConsumeFromMaster(masterTopic);
            consumeOnTopics(consumerInputTopics);
            stopConsumerOnTopics(consumerInputTopics);
            deleteConsumerTopics(consumerInputTopics);
            consumeUntilHaltOnTopics(consumerConsumedTopics);
            stopConsumerOnTopics(consumerConsumedTopics);
            deleteConsumerTopics(consumerConsumedTopics);
            programWrapper.run(stratumName);
            auto producerTopics = createProducerTopics(other::set_union(
                    stratumMetadata.outputRelationNames(), stratumMetadata.producedRelationNames()));
            produceOnTopics(producerTopics);
            produceHaltOnTopics(producerTopics);
            produceUntilEmpty();
            deleteProducerTopics(producerTopics);
        }
    }
